...
section_pattern = re.compile(r'Section (\d{1,3}(\.\d+)?(\+{0,1}))')

def chat(chat_history, user_input):
    user_input_with_ipc_question = f"{user_input} Which section of IPC covers this scenario? Tell me the exact section name ? "
    
    bot_response = qa_chain({"query": user_input_with_ipc_question})
    bot_response = bot_response['result']
    
    def highlight_sections(text):
        return section_pattern.sub(r'<span style="background-color: white; color: black;">\g<0></span>', text)
    
    response = ""
    for letter in ''.join(bot_response):
        response += letter + ""
    
    response_with_highlights = highlight_sections(response)
    
    yield chat_history + [(user_input_with_ipc_question, response_with_highlights)]
...
with gr.Blocks(theme=gr.themes.Soft()) as gradioUI:
